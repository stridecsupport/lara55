...
<head>
  ...
  <title>Quotation Edit Page</title>
   <!-- for table css -->
...
<tbody><tr id='row_{{$i}}'><td><input type='hidden' name='input_type[]' value='1'/><input type='checkbox' @if($child_quote_item->product_check == "1") checked @endif onclick=boxDisable('items',"{{$i}}",$(this),'0'); value='1' name='product_check[]'/><input type='hidden' value='0' id='checkboxhidden_{{$i}}' name=checkboxhidden[]/></td><td><select name='productid[]' onchange=getproductdetails("0","{{$i}}","{{$child_quote_item->product_id}}"); required><option value='0'>Select Items</option>@foreach($products as $productss)<option value='{{$productss->id}}' @if($productss->id == $child_quote_item->product_id) selected @endif>{{$productss->name}}</option>@endforeach</select></td><td><textarea id='description{{$i}}' name='product_description[]' required>{{$child_quote_item->item_description}}</textarea></td><td><input type='number' id='quantity{{$i}}' oninput=onchangequantity("{{$i}}"); name='quantity[]' value="{{$child_quote_item->quantity}}" required/></td><td><input type='text' id='cost_price{{$i}}' oninput=onchangecostprice("{{$i}}"); name='cost_price[]' value="{{$child_quote_item->product_cost}}" required/></td><td><input type='text' name='unit_price[]' value="{{$child_quote_item->unit_cost}}" oninput=onchangecalcalculation("{{$i}}"); id='unit_price{{$i}}' required/></td><td><input type='text' value="{{$child_quote_item->percentage}}" name='percentage[]' id='product_percentage{{$i}}' readonly required/></td><td><input id='margin_cost{{$i}}' value="{{$child_quote_item->margin}}"  type='text' name='margin[]' readonly required/></td><td><input type='text' name='amount[]' value="{{$child_quote_item->subtotal}}" id='product_amount{{$i}}' readonly required/></td><td><button type='button' onclick='deleterow("{{$i}}",rows,0);'>Delete</button></td></tr></tbody>
